...
namespace un::log {
    namespace detail {
        ...
        void set_runtime_clock_type(runtime_state& st, ClockType type) {
        ...
        void validate_global_config(const global_config& cfg) {
        ...
        backend::sink_ptr make_configured_sink(
                SinkType sink_type,
                const std::optional<fs::path>& filename,
                const std::optional<int>& output_fd,
                const std::optional<fs::path>& unix_dgram_path) {
        ...
        void add_configured_sink_locked(runtime_state& st, const channel_registration& registration) {
        ...
        queue_producer_registry& runtime_queue_registry() {
        ...
        void reconfigure_runtime_queue_producers(size_t thread_bufsize) {
        ...
        void ensure_global_config_locked(runtime_state& st) {
        ...
        runtime_queue_producer& register_runtime_queue_producer() {
            auto& st = access_state();
            auto& registry = runtime_queue_registry();
            std::lock_guard lock{registry.mutex};
        ...
        runtime_queue_producer& single_threaded_runtime_queue_producer();

        std::vector<runtime_queue_producer*> runtime_queue_producer_snapshot() {
            auto& st = access_state();
            ...
            snapshot.reserve(registry.producers.size());
            for (auto& producer : registry.producers)
                snapshot.push_back(producer.get());
            return snapshot;
        }
        ...
        void reset_runtime_queue_producers_for_test() {
            auto& st = access_state();
            if (st.single_threaded_producer)
        ...
        backend::time_context resolve_runtime_time_context(uint64_t timestamp, ClockType clock_type) {
        ...
        backend::log_entry make_runtime_log_entry(runtime_state& runtime, const backend::runtime_record_slot& slot) {
        ...
        std::vector<backend::sink_entry> sink_snapshot(runtime_state& runtime) {
            auto snapshot = std::vector<backend::sink_entry>{};
            std::lock_guard lock{runtime.runtime_mutex};
            snapshot.reserve(runtime.configured_sinks.size() + runtime.custom_sinks.size());
            for (auto& sink : runtime.configured_sinks)
                snapshot.push_back(sink.entry);
            snapshot.insert(snapshot.end(), runtime.custom_sinks.begin(), runtime.custom_sinks.end());
            return snapshot;
        }

        void emit_runtime_slot(runtime_state& runtime, const backend::runtime_record_slot& slot) {
        ...
        size_t drain_runtime_queues(runtime_state& runtime) {
            ...
            for (auto* producer : producer_snapshot) {
                ...
                drained += producer->queue().consume_all(
        ...
        void flush_runtime_sinks(runtime_state& runtime) {
            auto sinks = std::vector<backend::sink_ptr>{};
            {
                std::lock_guard lock{runtime.runtime_mutex};
                sinks.reserve(runtime.configured_sinks.size() + runtime.custom_sinks.size());
                for (auto& sink : runtime.configured_sinks)
                    sinks.push_back(sink.entry.sink);
                for (auto& sink : runtime.custom_sinks)
                    sinks.push_back(sink.sink);
            }
        ...
        void consumer_main(runtime_state& runtime) {
            auto observed_work = runtime.work_generation.load(std::memory_order_acquire);

            for (;;) {
                while (drain_runtime_queues(runtime) != 0) {
                }
        ...
        void ensure_consumer_started_locked(runtime_state& runtime) {
            if (runtime.consumer_started)
                return;

            runtime.stop_requested = false;
            runtime.consumer_thread = std::thread{[] { consumer_main(access_state()); }};
        ...
        void ensure_default_channel(runtime_state& st) {
